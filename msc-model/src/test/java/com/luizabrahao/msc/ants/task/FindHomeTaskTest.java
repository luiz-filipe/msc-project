...
public class FindHomeTaskTest {
	private static final Logger logger = LoggerFactory.getLogger(FindHomeTaskTest.class);
	...
	@Test
	public void findHomeTest() throws InterruptedException {
		...
		final List<AntAgent> agents = AntAgentFactory.produceBunchOfWorkers(20, "worker", grid[0][Integer.valueOf(nColumns / 2)]);
		...
		renderers.add(new ExploredSpaceRenderer(grid, "target/findHome - space explored.png", nColumns, nLines));
		renderers.add(new PheromoneRenderer(grid, "target/findHome - forage pheoromone.png", nColumns, nLines, ForageStimulusType.TYPE));
